...
## Python: `timeit` — More Features

```{.python code-line-numbers="1-5|8-13|15-16"}
# timeit.repeat: run the benchmark multiple times for statistics
times = timeit.repeat(
    lambda: [x**2 for x in range(n)],
    number=50, repeat=5         # 5 rounds of 50 calls each
)
print(f"best: {min(times):.4f}s  worst: {max(times):.4f}s")

# setup parameter: executed once before each round, not timed
t = timeit.timeit(
...
# Command line — quick one-liners without writing a script:
...
| | |
|-|-|
| `timeit()` | total time for *number* runs |
| `repeat()` | list of times → use `min()` for best-of |
| `-m timeit` | CLI for quick comparisons |
| `setup=` | one-time init, excluded from timing |
:::
:::
...
![](assets/adjlist_memory.png){width="90%"}

::: {.fragment}
![](assets/csr_memory.png){width="90%"}
:::
...
Roughly 2.8x fewer DRAM-class misses — the pipeline is no longer starved.
:::
...
```{.cmake code-line-numbers="1-3|5-8|10-13|15-18"}
