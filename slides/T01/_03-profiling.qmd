...
::: {.fragment}
::: {.platypus-tip}
Classify first with `stat`, then localize with `record`.
Starting with `record` means you're looking without knowing what for.
:::
:::
...
```text
 perf stat       →   classify      (what kind?)
 perf record     →   localize      (where?)
 perf annotate   →   line / asm    (which instruction?)
 edit code       →   fix
 perf stat       →   verify        (same workload, compare)
```

::: {.fragment}
::: {.platypus-info}
A change that lowers IPC while adding instructions, or shifts work into the kernel, is not a win. Compare **wall time and counters together** against a baseline.
:::
:::
